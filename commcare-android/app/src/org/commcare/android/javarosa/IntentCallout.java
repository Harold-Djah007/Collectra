...
public class IntentCallout implements Externalizable {
    ...
    private void storePointerToFileResponse(TreeReference ref, String responseValue, File destinationFile) {
        ...
        try {
            ...
            Log.w(TAG, "Collectra received a link to a file at " + responseValue + " to be included in the form, but it was not present on the phone!");

            //Wipe out any reference that exists
            formDef.setValue(null, ref);
        } catch (IOException ioe) {
            ioe.printStackTrace();
            Log.e(TAG,"Collectra failed to copy a binary input from an intent callout from: " + responseValue);
